...
def run_simulation_44(t_end=15.0, z_ref=1.0, vx_ref=0.5, phi_ref=0.0, psi_ref=0.0, verbose=True):
    ...
    for k in range(N):
        ...
        # FIX: Pass the clean vertical velocity (vz_hat = x_hat[16]) directly into the altitude D-term
        T_cmd = ctrl_alt.update(z_ref, x_hat[19], x_hat[16])
        
        th_ref = ctrl_vel.update(vx_ref, x_hat[14])
        
        tau = ctrl_att.update(phi_ref, th_ref, psi_ref, 
                              x_hat[11], x_hat[12], x_hat[13], 
                              x_hat[8],  x_hat[9],  x_hat[10])
        
        V = voltages_from_wrench(T_cmd, tau, x[4:8], p)
        x = euler_step(x, V, p)
